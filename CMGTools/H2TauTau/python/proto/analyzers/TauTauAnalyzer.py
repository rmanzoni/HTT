...
class TauTauAnalyzer( DiLeptonAnalyzer ):
    ...
    def declareHandles(self):
        ...
        if self.cfg_comp.isMC and ("DY" in self.cfg_comp.name or "Radion" in self.cfg_comp.name or "W" in self.cfg_comp.name or ("TTJets" in self.cfg_comp.name and 'emb' not in self.cfg_comp.name ) or "Higgs" in self.cfg_comp.name or "WW" in self.cfg_comp.name or "WZ" in self.cfg_comp.name or "ZZ" in self.cfg_comp.name):
        ...
        if self.cfg_comp.isMC and ("DY" in self.cfg_comp.name or "W" in self.cfg_comp.name and not "_new" in self.cfg_comp.name) :
    ...
    def process(self, iEvent, event):
        ...
        event.isSignal = self.selectionSequence(event, fillCounter=True, testLegSignal=True )
        if not event.isSignal :
          event.isBkg = self.selectionSequence(event, fillCounter=True, testLegSignal=False)
        if not (event.isSignal or event.isBkg) :
          return False
        ...
        if self.cfg_comp.isMC and ("DY"     in self.cfg_comp.name or \
                                   "Higgs"  in self.cfg_comp.name or \
                                   "WZJets" in self.cfg_comp.name or \
                                   "ZZJets" in self.cfg_comp.name ):
...
#                    }
    ...
    def selectionSequence(self, event, fillCounter, leg1IsoCut=None, leg2IsoCut=None, testLegSignal=True):
        ...
        for trig in matching.keys() :
          if fnm(trig,'HLT_IsoMu18_eta2p1_MediumIsoPFTau*_Trk*_eta2p1_v*') :
            event.l1TrigMatched_MuTau = matching[trig][0]
            event.l2TrigMatched_MuTau = matching[trig][1]      
        ...
        # testing leg1
        selDiLeptons = [ diL for diL in selDiLeptons if self.selectionTestLeg(diL.leg1(), testLegSignal) ]
        if len(selDiLeptons) == 0:
            return False
        else:
        ...
        # testing leg2 
        selDiLeptons = [ diL for diL in selDiLeptons if self.selectionTestLeg(diL.leg2(), testLegSignal) ]
        if len(selDiLeptons) == 0:
            return False
        else:
    ...
    def testLeg(self, leg):
        ...
        if iso == 'mva'  : #### Tau ID MVA
          test_leg_iso = leg.tauID("byRawIsoMVA") > 0.884 # 0.884       
        if iso == 'mva2' : #### Tau ID MVA2
          test_leg_iso = leg.tauID("byIsolationMVA2raw") > 0.90 # 0.90
        if iso == 'db3h' : #### Tau ID deltaBeta 3 hits
          test_leg_iso = leg.tauID("byCombinedIsolationDeltaBetaCorrRaw3Hits") < 1. #15. 
    ...
    def selectionTestLeg(self, leg, testLegSignal) :
      if testLegSignal : return self.testLeg(leg)
      else             : return self.testLooseLeg(leg)
    ...
    def trigMatched(self, event, leg, legName, trigpath):
        ...
        return triggerMatched(leg, triggerObjects, path, filter, dR2Max=0.5*0.5, dRMax=0.5, pdgIds=pdgIds )
    ...
    def splitDYJets( self, event, genParticles, myPair, require_status = True, bosons = [23,25,35,36]) : 
      ...
      genW  = [gen for gen in genParticles if abs(gen.pdgId())==23 and gen.status()==3]
      genZ  = [gen for gen in genParticles if abs(gen.pdgId())==24 and gen.status()==3]
      gentt = [gen for gen in genParticles if abs(gen.pdgId())==6  and gen.status()==3]

      event.hasW  = len(genW)==1
      event.hasZ  = len(genZ)==1
      event.hastt = len(genW)>=2 or len(gentt)==2
