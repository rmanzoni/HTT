...
class H2TauTauDataMC( AnalysisDataMC ):
    ...
    def _BuildHistogram(self, tree, comp, compName, varName, cut, layer ):
        ...
        hist.Sumw2()
        if varName == "l1DecayMode" :
          varName = "0*(l1DecayMode==0) + 1*(l1DecayMode==1) + 1*(l1DecayMode==2) + 2*(l1DecayMode==10) "
        if varName == "l2DecayMode" :
          varName = "0*(l2DecayMode==0) + 1*(l2DecayMode==1) + 1*(l2DecayMode==2) + 2*(l2DecayMode==10) "
	if varName == "tau1Mass":
	  varName = "sqrt(l1E*l1E-l1Px*l1Px-l1Py*l1Py-l1Pz*l1Pz)"
	if varName == "tau2Mass":
	  varName = "sqrt(l2E*l2E-l2Px*l2Px-l2Py*l2Py-l2Pz*l2Pz)"
	if varName == "bbMass"  :
          varName = "sqrt( (bjet1E  + bjet2E ) * (bjet1E  + bjet2E ) -\
                           ((bjet1Px + bjet2Px) * (bjet1Px + bjet2Px) +\
                            (bjet1Py + bjet2Py) * (bjet1Py + bjet2Py) +\
                            (bjet1Pz + bjet2Pz) * (bjet1Pz + bjet2Pz)) )"
	if varName == "radionMass"  :
          varName = "sqrt( (bjet1E  + bjet2E ) * (bjet1E  + bjet2E ) -  \
                           ((bjet1Px + bjet2Px) * (bjet1Px + bjet2Px) +  \
                            (bjet1Py + bjet2Py) * (bjet1Py + bjet2Py) +  \
                            (bjet1Pz + bjet2Pz) * (bjet1Pz + bjet2Pz)) ) +\
    ...
    def _ReadHistograms(self, directory):
        ...
        for layer, (compName, comp) in enumerate( self.selComps.iteritems() ) : 
            ...
            elif 'HiggsVH' in compName :
              ## need to scale by WH, VH and ttH relative cross sections
              #pattern = re.compile('Higgs(\D+)(\d+)')
              #m       = pattern.match( compName )
              #process = m.group(1)
              #mass    = float(m.group(2))
              #try:
              #  wh  = yrparser8TeV.get(mass)['WH'] ['sigma'] #/ (yrparser8TeV.get(mass)['WH']['sigma'] + yrparser8TeV.get(mass)['ZH']['sigma'] + yrparser8TeV.get(mass)['TTH']['sigma']) )
              #  zh  = yrparser8TeV.get(mass)['ZH'] ['sigma'] #/ (yrparser8TeV.get(mass)['WH']['sigma'] + yrparser8TeV.get(mass)['ZH']['sigma'] + yrparser8TeV.get(mass)['TTH']['sigma']) )
              #  tth = yrparser8TeV.get(mass)['TTH']['sigma'] #/ (yrparser8TeV.get(mass)['WH']['sigma'] + yrparser8TeV.get(mass)['ZH']['sigma'] + yrparser8TeV.get(mass)['TTH']['sigma']) )
              #except KeyError:
              #  wh_up    = yrparser8TeV.get(mass+1.)['WH'] ['sigma']
              #  zh_up    = yrparser8TeV.get(mass+1.)['ZH'] ['sigma']
              #  tth_up   = yrparser8TeV.get(mass+1.)['TTH']['sigma']
              #  wh_down  = yrparser8TeV.get(mass-1.)['WH'] ['sigma']
              #  zh_down  = yrparser8TeV.get(mass-1.)['ZH'] ['sigma']
              #  tth_down = yrparser8TeV.get(mass-1.)['TTH']['sigma']
              #  wh  =  0.5 * (wh_up  + wh_down ) #/ ( 0.5 * (wh_up + wh_down) + 0.5 * (zh_up + zh_down) + 0.5 * (tth_up + tth_down) ) )
              #  zh  =  0.5 * (zh_up  + zh_down ) #/ ( 0.5 * (wh_up + wh_down) + 0.5 * (zh_up + zh_down) + 0.5 * (tth_up + tth_down) ) )
              #  tth =  0.5 * (tth_up + tth_down) #/ ( 0.5 * (wh_up + wh_down) + 0.5 * (zh_up + zh_down) + 0.5 * (tth_up + tth_down) ) )
              #
              #comp.xSectionWH  = wh
              #comp.xSectionZH  = zh
              #comp.xSectionttH = tth
              print 'comp.xSectionWH  ',comp.xSectionWH  
              print 'comp.xSectionZH  ',comp.xSectionZH  
              print 'comp.xSectionttH ',comp.xSectionttH 
              #self.eventWeight = my_evtweight + ' * ' + str(wh)
              self._BuildHistogram(tree, comp, compName.replace('HiggsVH','HiggsWH' ) , self.varName, self.cut + ' & hasW'  , layer)
              #self.eventWeight = my_evtweight + ' * ' + str(zh)
              self._BuildHistogram(tree, comp, compName.replace('HiggsVH','HiggsZH' ) , self.varName, self.cut + ' & hasZ'  , layer)
              #self.eventWeight = my_evtweight + ' * ' + str(tth)
              self._BuildHistogram(tree, comp, compName.replace('HiggsVH','HiggsttH') , self.varName, self.cut + ' & hastt' , layer)              
              self.weights[compName.replace('HiggsVH','HiggsWH' )] = self.weights[compName]
              self.weights[compName.replace('HiggsVH','HiggsZH' )] = self.weights[compName]
              self.weights[compName.replace('HiggsVH','HiggsttH')] = self.weights[compName]
    ...
    def _InitPrefs(self):
        ...
        self.histPref['Radion*']          = {'style':sHTT_Higgs , 'layer':1001 , 'legend':None                   }
