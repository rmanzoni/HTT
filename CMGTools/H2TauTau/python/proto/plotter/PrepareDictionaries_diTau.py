...
def lists(just125,susy,run2012, radion) :

  if susy     : signal = ['HiggsSUSYBB','HiggsSUSYGluGlu']
  elif radion : signal = ['Radion']
  else        : signal = ['HiggsGGH','HiggsVBF','HiggsVH']
  #else        : signal = ['HiggsGGH','HiggsVBF','HiggsWH','HiggsZH','HiggsttH']
  ...
  #if susy : bkg.extend(['HiggsGGH125','HiggsVBF125','HiggsWH125','HiggsZH125','HiggsttH125','TTJets_emb']) 
  else    : pass                                      
  ...
  if radion  : masses = [300,500,700]
  
  return signal, bkg, data2012, embed2012, masses

def componentsWithData (selComps, weights, susy, radion) :

  signal, bkg, data2012, embed2012, masses = lists(just125,susy,run2012, radion)
...
def componentsWithOutData (selComps, weights, susy, radion) :
...
def componentsWithOutSignal (selComps, weights, susy, radion) :
