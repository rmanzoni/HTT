...
##            self.printTriggerObject( event.triggerObject )
        ...
        if hltPath is not None :
          ...
          ## RICCARDO: this is useful for the matching
          ## --DEFAULT-- if the first trigger is fired the trigger obj of the following triggers are not saved
          if not self.cfg_ana.keepAllTriggerObjects :
            event.triggerObjects = selTriggerObjects( trigObjs, hltPath )
          ## --FOR CHECKS-- save all the trigger obj of all the triggers fired among self.cfg_comp.triggers
          else :
            #import pdb ; pdb.set_trace()
            event.triggerObjects = []          
            for hltPath in self.cfg_comp.triggers :
              event.triggerObjects.extend( selTriggerObjects( trigObjs, hltPath ) )
            #import pdb ; pdb.set_trace()
