...
rePatMassRadion = re.compile('_M(\d+)_GENSIM') ## RM patch ad hoc
...
def findAlias(path_name, aliases):
    ...
    if name is None:
        return None
    if 'Radion' in path_name : match = rePatMassRadion.search(path_name)
    else                     : match = rePatMass.search(path_name)
